...
class SchedulesPage(BasePage):
    ...
    def render(self):
        ...
        if not schedule_generated:
            st.info(
                "No schedules generated yet. Please generate schedules from the Dashboard."
            )
            st.markdown("---")
        ...
        tab1, tab2 = st.tabs(
            ["📅 Year-wise Schedules", "🏢 Room-wise Schedules & Availability"]
        )
    ...
    def render_year_wise_schedules(self, schedules):
        ...
        for year in years:
            ...
            year_schedules = [
                (data, flag) for data, flag in schedules if int(data["year"]) == year
            ]
    ...
    def render_room_wise_schedules(self, schedules):
        ...
        for room_data in sorted(all_rooms, key=lambda x: str(x["RoomID"])):
            ...
            room_schedules = [
                (data, flag) for data, flag in schedules if data["room"] == room_id
            ]
